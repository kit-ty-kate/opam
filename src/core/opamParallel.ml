...
module Make (G : G) = struct
  ...
  let aux_map ~jobs ~command ?(dry_run=false) ?(pools=[]) g =
    ...
    let rec loop
      ...
      let fail node error =
        ...
        let remaining =
          G.Topological.fold (fun n remaining ->
              if M.mem n results || List.mem_assoc ~eq:G.V.equal n errors then remaining
              else n::remaining)
            g [] in
          ...
          (slog
             (fun pools ->
                ...
                OpamStd.List.concat_map " " (fun (pool, jobs) ->
                  let nslots =
                    OpamStd.Option.of_Not_found (List.assoc ~eq:S.equal pool) slots
                  in
                  Printf.sprintf "%s/%d"
                    (match nslots with
                     | None -> "-"
      ...
      in
      let n,cont = List.assoc ~eq:OpamProcess.equal process processes in
...
module MakeGraph (X: VERTEX) = struct
  ...
  let of_json : t OpamJson.decoder = function
    | `O dict ->
      begin try
          let vertices_json = match List.assoc ~eq:String.equal "vertices" dict with
            | `O vertices -> vertices
            | _ -> raise Not_found in
          let edges_json = match List.assoc ~eq:String.equal "edges" dict with
            | `A edges -> edges
            | _ -> raise Not_found in
          let vertex_map =
          ...
          let edges =
            let int_of_jsonstring = function
              ...
              | _ -> raise Not_found in
            let find kj = List.assoc ~eq:Int.equal (int_of_jsonstring kj) vertex_map in
            let edge_of_json = function
              | `O dict ->
                let src = find (List.assoc ~eq:String.equal "src" dict) in
                let label = () in
                let dst = find (List.assoc ~eq:String.equal "dst" dict) in
