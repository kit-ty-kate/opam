...
let menu ?default ?unsafe_yes ?yes ~no ~eq ~options fmt =
  ...
  let rec menu default =
    ...
    msg "%s" text;
    let select a =
      msg "%s\n" (List.assoc ~eq a options_nums); a
    in
    let default_s = List.assoc ~eq default options_nums in
    let no_s = List.assoc ~eq no options_nums in
