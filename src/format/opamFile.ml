...
module OPAMSyntax = struct
  ...
  let cleanup_name _opam_version ~pos name =
    match OpamPackage.of_filename (OpamFilename.of_string pos.filename) with
  ...
  let cleanup_version _opam_version ~pos version =
    match OpamPackage.of_filename (OpamFilename.of_string pos.filename) with
