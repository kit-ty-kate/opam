...
let get_root_raw root repo_locations name =
  match OpamRepositoryName.Map.find_opt name repo_locations with
  | Some repo_root -> repo_root
  | None -> OpamFilename.D (OpamRepositoryPath.root root name)

let get_root rt name =
  get_root_raw rt.repos_global.root rt.repo_locations name

let get_repo_root rt repo =
  get_root_raw rt.repos_global.root rt.repo_locations repo.repo_name
    ...
    let repofiles, opams =
      OpamRepositoryName.Map.fold (fun name url (defs, opams) ->
          ...
          let repo_def, repo_opams =
            load_repo repo (get_root_raw gt.root repo_locations name)
          in
