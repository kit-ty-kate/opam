...
let apply_repo_update repo repo_root = function
  | Update_full d ->
    log "%a: applying update from scratch at %a"
      ...
      (slog OpamRepositoryRoot.Dir.to_string) d;
    OpamRepositoryRoot.Dir.rmdir repo_root;
    if OpamRepositoryRoot.Dir.is_symlink d then
      (OpamRepositoryRoot.Dir.copy ~src:d ~dst:repo_root;
       OpamRepositoryRoot.Dir.rmdir d)
    else
      OpamRepositoryRoot.Dir.move ~src:d ~dst:repo_root;
  ...
  | Update_patch f ->
    ...
    let preprocess =
      ...
      | _ -> true
    in
    (OpamFilename.patch ~preprocess f (OpamRepositoryRoot.Dir.to_dir repo_root) @@+ function
      | Some e ->
...
let cleanup_repo_update upd =
  if not (OpamConsole.debug ()) then
    match upd with
    | Update_full d -> OpamRepositoryRoot.Dir.rmdir d
