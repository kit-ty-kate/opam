...
let remove rt name =
  ...
  OpamRepositoryRoot.Dir.rmdir (OpamRepositoryPath.root rt.repos_global.root name);
  OpamRepositoryRoot.Tar.remove (OpamRepositoryPath.tar rt.repos_global.root name);
  rt
  ...
  let repo =
    ...
    with Not_found ->
      OpamConsole.error_and_exit `Not_found "No repository %s found"
        (OpamRepositoryName.to_string name);
  in
  OpamRepositoryRoot.Dir.cleandir (OpamRepositoryPath.root rt.repos_global.root name);
  OpamRepositoryRoot.Tar.remove (OpamRepositoryPath.tar rt.repos_global.root name);
