    ...
    | Some `set_url, (name :: url :: security) ->
      ...
      let backup =
        ...
        if OpamRepositoryRoot.Tar.exists tar then
          (let target = OpamRepositoryRoot.Tar.clone_name_in tmp_dir tar in
          ...
          (let dir = OpamRepositoryPath.root gt.root name in
           if not (OpamRepositoryRoot.Dir.exists dir) then
             OpamConsole.error_and_exit `Internal_error
               ...
               (OpamRepositoryName.to_string name);
           let target = OpamRepositoryRoot.Dir.clone_name_in tmp_dir dir in
    ...
    if repos then
      (OpamFilename.with_flock `Lock_write (OpamPath.repos_lock gt.root)
       ...
       OpamRepositoryName.Set.iter (fun r ->
           ...
           rmdir (OpamRepositoryRoot.Dir.to_dir (OpamRepositoryPath.root root r));
