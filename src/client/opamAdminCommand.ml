...
let index_command cli =
  ...
  let cmd global_options urls_txt () =
    ...
    let repo_stamp =
      ...
      in
      match OpamUrl.guess_version_control (OpamRepositoryRoot.Dir.to_string repo_root)
      with
      | None -> date ()
      | Some vcs ->
        let module VCS = (val OpamRepository.find_backend_by_kind vcs) in
        match OpamProcess.Job.run (VCS.revision (OpamRepositoryRoot.Dir.to_dir repo_root)) with
        | None -> date ()
    ...
    if urls_txt <> `no_urls_txt then
      (OpamConsole.msg "Generating urls.txt...\n";
       ...
       (if urls_txt = `full_urls_txt then
          OpamFilename.rec_files (OpamRepositoryRoot.Dir.subdir repo_root "compilers") @
       ...
       List.fold_left (fun set f ->
           ...
           let attr = OpamRepositoryRoot.Dir.to_attribute repo_root f in
...
let cache_command cli =
  ...
  let cmd global_options cache_dir no_repo_update link jobs recheck () =
    ...
    let cache_dir_url = OpamRepositoryRoot.Dir.remove_prefix repo_root cache_dir in
...
let upgrade_command cli =
  ...
  let cmd global_options clear_cache create_mirror () =
    ...
    if clear_cache then OpamAdminRepoUpgrade.clear_cache ()
      ...
      | None ->
        OpamAdminRepoUpgrade.do_upgrade (OpamRepositoryRoot.Dir.of_dir (OpamFilename.cwd ()));
        ...
        then
          OpamConsole.note
            "Indexes need updating: you should now run:\n\
             \n\
            \  opam admin index"
      | Some m -> OpamAdminRepoUpgrade.do_upgrade_mirror (OpamRepositoryRoot.Dir.of_dir (OpamFilename.cwd ())) m
  in
  OpamArg.mk_command  ~cli OpamArg.cli_original command ~doc ~man
    Term.(const cmd $ global_options cli $
...
let lint_command cli =
  ...
  let cmd global_options short list incl excl ign warn_error () =
    OpamArg.apply_global_options cli global_options;
    let repo_root = OpamRepositoryRoot.Dir.of_dir (OpamFilename.cwd ()) in
    if not (OpamFilename.exists_dir (OpamRepositoryRoot.Dir.subdir repo_root "packages"))
    then
        OpamConsole.error_and_exit `Bad_arguments
          "No repository found in current directory.\n\
...
let list_command cli =
  ...
  let cmd
    ...
    in
    let st = get_virtual_switch_state (OpamRepositoryRoot.Dir.of_dir (OpamFilename.cwd ())) env in
