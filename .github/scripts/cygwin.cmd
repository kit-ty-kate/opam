...
:: PATH is only set when we know we're storing the cache because of an
...
rem Keep Windows tar main PATH over Cygwin
rem We can't just put %SYSTEM_ROOT%\System32 back in because that would pull in bash
::md %CYGWIN_ROOT%\wbin
::copy %SYSTEM_ROOT%\tar.exe %CYGWIN_ROOT%\wbin\tar.exe
::echo %CYGWIN_ROOT%\wbin>> %GITHUB_PATH%

:: Prevent tar and git being used from PATH outside Cygwin
:: XXX Eliminate duplication with create phase
...
:: XXX Would like to eliminate the need for libicu-devel (i.e. have another way of writing the symlinks correctly)
...
:: Prevent tar and git being used from PATH outside Cygwin
:: XXX Eliminate duplication with restore phase
%CYGWIN_ROOT%\bin\bash -lc "cd /usr/bin ; mv tar cygtar ; echo -n '!<symlink>' > tar ; echo -ne 'cygtar.exe\000' | uconv -t UTF16LE --add-signature >> tar ; mv git cyggit ; echo -n '!<symlink>' > git ; echo -ne 'cyggit.exe\000' | uconv -t UTF16LE --add-signature >> git ; chattr -f +s git tar"
%CYGWIN_ROOT%\bin\bash -lc "ls -l /usr/bin"
